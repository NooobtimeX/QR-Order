<template>
    ...
    <div class="sm:mr-64">
      ...
      <table class="min-w-full">
        ...
        <tbody class="divide-y divide-gray-200 bg-white">
          <tr v-for="(order, index) in sortedOrders" :key="index" @click="selectOrder(order)"
            ...
            <td class="whitespace-nowrap">
              <span class="rounded-xl px-2 py-1 text-sm" :class="{
  ...
  <aside
    ...
    <div v-if="selectedOrder" class="p-4">
      ...
      <select v-model="selectedOrder.status" @change="updateOrderStatus" class="rounded-xl text-black">
  ...
  :key="index"
  class="transition duration-150 bg-green-300 text-black border border-gray-500 rounded p-4 mb-4"
>
  {{ item.name }} {{ item.price }}
  <div v-for="(option, index) in item.options" :key="index" class="mt-2 text-gray-700">
    &nbsp;{{ option }}
  </div>
</div>
